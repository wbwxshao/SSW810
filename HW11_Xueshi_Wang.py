...
class Repository:
    ...
    def feed_instructor(self):
        ...
        for row in db.execute(sql_command):
            self.instructor[row[0]] = instructor(row[0], row[1], row[2])
        for row in db.execute(sql_command):    
            self.instructor[row[0]].add_num(row[3], row[4])     
...
class instructor:
    ...
    def add_num(self, course_name, num):
        """FOR hw 11"""
        self.Course[course_name] = num
